/* global petermason_color_schemes, petermason_dependencies, Color */
...
( function( api ) {
	...
	for (var i in petermason_color_schemes) {
		cssTemplate[i] = wp.template( 'petermason-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'petermason-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {
		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+petermason_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(petermason_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', petermason_customizer_update_load_fonts);		
		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', petermason_customizer_actions);
		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			petermason_customizer_check_dependencies(jQuery(this));
		});
		// Refresh color controls
		petermason_customizer_change_color_scheme(api('color_scheme_editor')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		petermason_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		petermason_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function petermason_customizer_check_dependencies(cont) {
		"use strict";
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in petermason_dependencies) {
				if (fld == id) {
					depend = petermason_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('petermason_options_no_use');
				} else {
					jQuery(this).hide().addClass('petermason_options_no_use');
				}
			}
		});
	}
	...
	function petermason_customizer_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme_editor') {
			petermason_customizer_change_color_scheme(val);

		} else if (updateCSS) {
			// Check colors in scheme
			var simple = api('color_settings')()=='simple';
			for (opt in petermason_color_schemes['default'].colors) {
				if (opt == id) {
					// Store new value in the color table
					petermason_customizer_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							petermason_customizer_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							petermason_customizer_update_color_scheme('alter_hover', val);
						}
					}
					processed = true;
					break;
			...
			// Check theme fonts
			if (!processed) {
				for (opt in petermason_theme_fonts) {
					for (rule in petermason_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							petermason_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) petermason_customizer_update_css();
		}
	...
	// Actions buttons
	function petermason_customizer_actions(e) {
		"use strict";
	...
	// Store new value in the color table
	function petermason_customizer_update_color_scheme(opt, value) {
		"use strict";
		petermason_color_schemes[api('color_scheme_editor')()].colors[opt] = value;
		api('scheme_storage').set(petermason_serialize(petermason_color_schemes))
	}
	
	// Store new value in the theme fonts
	function petermason_customizer_update_theme_fonts(opt, rule, value) {
		"use strict";
		petermason_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function petermason_customizer_update_load_fonts() {
		"use strict";
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=petermason_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in petermason_theme_fonts) {
	...
	function petermason_customizer_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in petermason_color_schemes[value].colors) {
			if (api(opt) == undefined) continue;
			api( opt ).set( petermason_color_schemes[value].colors[opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', petermason_color_schemes[value].colors[opt] )
				.wpColorPicker( 'defaultColor', petermason_color_schemes[value].colors[opt] );
		}
		updateCSS = true;
		petermason_customizer_update_css();
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function petermason_customizer_update_css() {
		"use strict";
		...
		var fonts = petermason_customizer_add_theme_fonts(petermason_theme_fonts);

		// Make styles and add into css
		css += petermason_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in petermason_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in petermason_color_schemes[scheme].colors) {
				colors[i] = petermason_color_schemes[scheme].colors[i];
			}
	...
	function petermason_customizer_add_theme_colors(colors) {
		...
		colors.bg_color_009 = Color( colors.bg_color ).toCSS( 'rgba', 0.09 );
		...
		colors.alter_bg_color_057 = Color( colors.alter_bg_color ).toCSS( 'rgba', 0.57 );
		...
		colors.text_hover_01 = Color( colors.text_hover ).toCSS( 'rgba', 0.1 );
		colors.text_hover_02 = Color( colors.text_hover ).toCSS( 'rgba', 0.2 );
		colors.text_hover_03 = Color( colors.text_hover ).toCSS( 'rgba', 0.3 );
		colors.text_hover_05 = Color( colors.text_hover ).toCSS( 'rgba', 0.5 );
		colors.text_dark_07 = Color( colors.text_dark ).toCSS( 'rgba', 0.7 );
		colors.text_dark_09 = Color( colors.text_dark ).toCSS( 'rgba', 0.9 );
		colors.text_link_02 = Color( colors.text_link ).toCSS( 'rgba', 0.2 );
		colors.text_link_05 = Color( colors.text_link ).toCSS( 'rgba', 0.5 );
		colors.text_link_07 = Color( colors.text_link ).toCSS( 'rgba', 0.7 );
		colors.input_text_025 = Color( colors.input_text ).toCSS( 'rgba', 0.25 );
		colors.inverse_text_05 = Color( colors.inverse_text ).toCSS( 'rgba', 0.5 );
		colors.inverse_dark_05 = Color( colors.inverse_dark ).toCSS( 'rgba', 0.5 );
		return colors;
	}
	...
	function petermason_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + petermason_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + petermason_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + petermason_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function petermason_customizer_prepare_css_value(val) {
		"use strict";
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
	...
	function petermason_customizer_prepare_html_value(val) {
		"use strict";
