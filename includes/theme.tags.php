...
if ( !function_exists('petermason_need_page_title') ) {
	function petermason_need_page_title() {
		return !is_front_page() && apply_filters('petermason_filter_need_page_title', true);
	}
}
...
if ( !function_exists('petermason_show_layout') ) {
	function petermason_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('petermason_get_logo_image') ) {
	function petermason_get_logo_image($type='') {
		$logo_image = '';
		if (petermason_get_retina_multiplier(2) > 1)
			$logo_image = petermason_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = petermason_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}
...
if ( !function_exists('petermason_show_post_meta') ) {
	function petermason_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
			...
			if ( !empty($args['categories'])) {	
				$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('petermason_filter_get_post_categories', '');
				if (!empty($cats)) {
					?>
					<span class="post_meta_item post_categories"><?php petermason_show_layout($cats); ?></span>
					<?php
				}
			}
			// Post date
			if ( !empty($args['date'])) {		
				$dt = apply_filters('petermason_filter_get_post_date', petermason_get_date());
				if (!empty($dt)) {
					?>
			...
			// Post counters
			petermason_show_layout(petermason_get_post_counters($args['counters']));

			// Socials share
			if ( !empty($args['share']) ) {
				petermason_show_share_links(array(
						'type' => 'drop',
						'caption' => esc_html__('Share', 'petermason'),
			...
			// Edit page link
			if ( !empty($args['edit']) ) {
				edit_post_link( esc_html__( 'Edit', 'petermason' ), '<span class="post_meta_item post_edit icon-pencil">', '</span>' );
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('petermason_show_post_featured') ) {
	function petermason_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => petermason_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : petermason_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		...
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				...
				$image = petermason_get_post_image();
				if (!empty($image)) 
					$image = petermason_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = petermason_get_no_image_placeholder();
			$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . petermason_add_inline_style('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="//schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$petermason_attr = petermason_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
					...
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
							...
							?><img<?php if ($args['seo']) echo ' itemprop="url"'; ?> src="<?php echo esc_url(petermason_get_no_image_placeholder()); ?>" alt="<?php the_title_attribute(); ?>"><?php
						}
					}
					?>
				</div><!-- .post_featured -->
			...
			if ($has_thumb || !empty($args['show_no_image']) || (!$args['thumb_only'] && in_array($post_format, array('gallery', 'image', 'audio', 'video')))) {
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						petermason_hovers_add_icons($args['hover']);
					}
					...
					if ( ($output = petermason_build_slider_layout(array('thumb_size' => $thumb_size, 'controls' => 'yes', 'pagination' => 'yes'))) != '' )
						petermason_show_layout($output);
					...
					if (!empty($image)) {
						...
						<div class="mask"></div>
						<?php 
						petermason_hovers_add_icons($args['hover'], array('image' => $image));
					}
				} else if (!empty($args['show_no_image'])) {
					?>
					<img src="<?php echo esc_url(petermason_get_no_image_placeholder()); ?>" alt="<?php the_title_attribute(); ?>">
					<div class="mask"></div>
					<?php 
					petermason_hovers_add_icons($args['hover']);
				}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = petermason_get_post_video($post_content, false);
						if (empty($video))
							$video = petermason_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = petermason_array_get_first($videos, false);
								}
							}
						}
						if (!empty($video)) {
							if ( $has_thumb ) {
								$video = petermason_make_video_autoplay($video);
								?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php
							}
							?><div class="post_video video_frame"><?php
								if ( !$has_thumb ) {
									petermason_show_layout($video);
								}
							?></div><?php
						}
					...
					// Put audio over the thumb
						...
						$audio = petermason_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = petermason_get_post_iframe($post_content, false);
						if (empty($audio)) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', get_the_content() );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($audios) && is_array($audios)) {
									$audio = petermason_array_get_first($audios, false);
								}
							}
						}
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php
								...
								$media_author = petermason_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = petermason_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !petermason_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php petermason_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !petermason_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php petermason_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								petermason_show_layout($audio);
							?></div><?php
						}
					}
				}
				...
				petermason_show_layout($post_info);
				?></div><?php
			}
		}
	}
...
if ( !function_exists('petermason_get_no_image_placeholder') ) {
	function petermason_get_no_image_placeholder($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = petermason_get_theme_option( 'no_image' );
			if (empty($img)) $img = petermason_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('petermason_add_bg_in_post_nav') ) {
	function petermason_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = petermason_get_no_image_placeholder();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), petermason_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), petermason_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'petermason-main', $css );
	}
}

// Show related posts
if ( !function_exists('petermason_show_related_posts') ) {
	function petermason_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			'numberposts' => max(2, min(4, petermason_get_theme_option('related_posts'))),
			'orderby' => 'post_date',
			'order' => 'DESC',
			'post_type' => 'post',
			'post_status' => 'publish',
			...
			<section class="related_wrap">
				<h3 class="section title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'petermason');
				?></h3>
...
if ( !function_exists('petermason_show_portfolio_posts') ) {
	function petermason_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? petermason_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = petermason_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = petermason_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the petermason_ajax_get_posts action
if ( !function_exists( 'petermason_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_petermason_ajax_get_posts',			'petermason_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_petermason_ajax_get_posts',	'petermason_ajax_get_posts_callback');
	function petermason_ajax_get_posts_callback() {
		...
		if ((int) $id > 0) {
			...
			petermason_storage_set('blog_mode', 'blog');
			petermason_storage_set('options_meta', get_post_meta($id, 'petermason_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => petermason_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'petermason');
		}
...
// Show pagination
if ( !function_exists('petermason_show_pagination') ) {
	function petermason_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = petermason_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'petermason' ),
				'next_text' => esc_html__( '>', 'petermason' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'petermason' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'petermason'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'petermason') : esc_html__('Newest posts', 'petermason') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'petermason') : esc_html__('Older posts', 'petermason'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'petermason_single_comments_field' ) ) {
    function petermason_single_comments_field( $args ) {
